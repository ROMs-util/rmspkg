...
function Get-SafeRelativePath {
    ...
    # Separator-rooted / UNC rejection. Normalize the separator form first
    # (the probe is for the prefix test ONLY - $Relative itself is untouched
    # and the resolve/containment logic below is unchanged) so a leading
    # backslash or slash is detected deterministically no matter how the
    # value was authored or decoded. A single leading separator is rejected
    # too: it is drive-relative on Windows ("\Windows" resolves against the
    # current drive's root), so refusing it matches the guard's stated
    # "relative path only" contract instead of relying on Join-Path to
    # happen to swallow it into Root.
    $uncProbe = $Relative.Replace('\', '/')
    if ($uncProbe.StartsWith('/')) {
